...
<div class="min-h-full">
    ...
    <main class="py-10">
        <div class="px-4 sm:px-6 lg:px-8">
                ...
                <nav class="flex mb-4" aria-label="Breadcrumb">
                    <ol class="flex items-center space-x-4">
                        ...
                        <li>
                            <div class="flex items-center">
                                <i class="flex-shrink-0 w-5 h-5 text-gray-300 fas fa-chevron-right"></i>
                                <span class="ml-4 text-sm font-medium text-gray-500">Editar Item</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-2xl font-bold leading-6 text-gray-900">Editar Item do Menu</h1>
                <p class="mt-2 text-sm text-gray-700">Edite o item "{{ $menuItem->name }}" da categoria "{{ $menuCategory->name }}"</p>
            </div>
            ...
            <form action="{{ route('restaurant.menu.items.update', $menuItem) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('POST')
                    ...
                    <div class="px-6 py-6 space-y-6">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="number" name="preparation_time" id="preparation_time" value="{{ old('preparation_time', $menuItem->preparation_time) }}" required min="1"
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm"
                                       placeholder="15">
                            </div>
                        </div>
                        ...
                        <div>
                            <label for="menu_category_id" class="block text-sm font-medium text-gray-700">Categoria *</label>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm"
                                      placeholder="Descrição detalhada do item, ingredientes principais, etc.">{{ old('description', $menuItem->description) }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                            ...
                            <div>
                                ...
                                <div class="relative mt-1 rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="discount_price" id="discount_price" value="{{ old('discount_price', $menuItem->discount_price) }}" min="0" step="0.01"
                                           class="block w-full pl-12 border-gray-300 rounded-md focus:border-orange-500 focus:ring-orange-500 sm:text-sm"
                                           placeholder="0.00">
                                </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="calories" id="calories" value="{{ old('calories', $menuItem->calories) }}" min="0"
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm"
                                       placeholder="250">
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-lg shadow">
                    ...
                    <div class="px-6 py-6 space-y-6">
                        <div>
                            ...
                            <input type="hidden" name="ingredients" id="ingredients_hidden" value="{{ old('ingredients') ? json_encode(old('ingredients')) : json_encode($menuItem->ingredients ?? []) }}">
                        ...
                        <div>
                            ...
                            <input type="hidden" name="allergens" id="allergens_hidden" value="{{ old('allergens') ? json_encode(old('allergens')) : json_encode($menuItem->allergens ?? []) }}">
                ...
                <div class="bg-white rounded-lg shadow">
                    ...
                    <div class="px-6 py-6">
                        ...
                        @if($menuItem->image)
                        <div class="mb-4">
                            <label class="block mb-2 text-sm font-medium text-gray-700">Imagem Atual</label>
                            <div class="flex items-center space-x-4">
                                <img src="{{ asset('storage/' . $menuItem->image) }}" alt="{{ $menuItem->name }}" class="object-cover w-32 h-32 border border-gray-300 rounded-lg">
                        ...
                        <div>
                            <label for="image" class="block text-sm font-medium text-gray-700">{{ $menuItem->image ? 'Nova Foto do Prato' : 'Foto do Prato' }}</label>
                                ...
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-gray-600">
                                        <label for="image" class="relative font-medium text-orange-600 bg-white rounded-md cursor-pointer hover:text-orange-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-orange-500">
                                            <span>{{ $menuItem->image ? 'Trocar foto' : 'Upload de foto' }}</span>
                            ...
                            <div class="overflow-hidden bg-white border border-gray-300 rounded-lg">
                                <div class="bg-gray-100 aspect-w-16 aspect-h-9">
                                    <div id="preview-image" class="flex items-center justify-center w-full h-48 bg-gray-100">
                                        @if($menuItem->image)
                                            <img src="{{ asset('storage/' . $menuItem->image) }}" alt="{{ $menuItem->name }}" class="object-cover w-full h-48">
                                        @else
                                ...
                                <div class="p-4">
                                    <div class="flex items-start justify-between">
                                        <h3 id="preview-name" class="text-lg font-medium text-gray-900">{{ $menuItem->name }}</h3>
                                        <div id="preview-badges" class="flex items-center space-x-1"></div>
                                    </div>
                                    <p id="preview-description" class="mt-2 text-sm text-gray-600">{{ $menuItem->description ?: 'Descrição do item' }}</p>
                                    <div class="flex items-center justify-between mt-3">
                                        <div class="flex items-center space-x-2">
                                            <span id="preview-price" class="text-lg font-bold text-gray-900">MT {{ number_format($menuItem->price, 2) }}</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        if (featuredInput.checked) {
            previewBadges.innerHTML += '<span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">⭐</span>';
        }
    ...
    [vegetarianInput, veganInput, spicyInput, availableInput, featuredInput].forEach(input => {
        if (input) {
...
function setupTagsInput(type) {
    ...
    function updateTagsDisplay() {
        ...
        tags.forEach(tag => {
            ...
            tagElement.innerHTML = `
                ${tag}
                <button type="button" onclick="removeTagFromType('${type}', '${tag}')" class="ml-1.5 inline-flex items-center justify-center w-4 h-4 text-orange-400 hover:text-orange-600">
    ...
    window[`removeTagFromType`] = function(tagType, tagToRemove) {
        if (tagType === type) {
