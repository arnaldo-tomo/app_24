    ...
    <main class="py-10">
        <div class="px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-2xl font-bold leading-6 text-gray-900">Editar Restaurante</h1>
                <p class="mt-2 text-sm text-gray-700">Atualize as informações do restaurante {{ $restaurant->name }}</p>
            </div>
            ...
            <form action="{{ route('admin.restaurants.update', $restaurant) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="px-6 py-6 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">{{ old('description', $restaurant->description) }}</textarea>
                        </div>
                    ...
                    <div class="px-6 py-6 space-y-6">
                        <div>
                            ...
                            <textarea name="address" id="address" rows="2" required
                                      class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">{{ old('address', $restaurant->address) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <label for="latitude" class="block text-sm font-medium text-gray-700">Latitude</label>
                                <input type="number" name="latitude" id="latitude" value="{{ old('latitude', $restaurant->latitude) }}" step="any"
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                            </div>

                            <div>
                                <label for="longitude" class="block text-sm font-medium text-gray-700">Longitude</label>
                                <input type="number" name="longitude" id="longitude" value="{{ old('longitude', $restaurant->longitude) }}" step="any"
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-6 space-y-6">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                            <div>
                                ...
                                <div class="relative mt-1 rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="delivery_fee" id="delivery_fee" value="{{ old('delivery_fee', $restaurant->delivery_fee) }}"
                            ...
                            <div>
                                ...
                                <div class="relative mt-1 rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="minimum_order" id="minimum_order" value="{{ old('minimum_order', $restaurant->minimum_order) }}"
                                           min="0" step="0.01" required
                                           class="block w-full pl-12 border-gray-300 rounded-md focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input type="number" name="delivery_time_min" id="delivery_time_min" value="{{ old('delivery_time_min', $restaurant->delivery_time_min) }}"
                                       min="1" required
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="delivery_time_max" id="delivery_time_max" value="{{ old('delivery_time_max', $restaurant->delivery_time_max) }}"
                                       min="1" required
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                            </div>
                        </div>
                    ...
                    <div class="px-6 py-6 space-y-6">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input type="time" name="opening_time" id="opening_time" value="{{ old('opening_time', $restaurant->opening_time) }}" required
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                            </div>

                            <div>
                                <label for="closing_time" class="block text-sm font-medium text-gray-700">Horário de Fechamento *</label>
                                <input type="time" name="closing_time" id="closing_time" value="{{ old('closing_time', $restaurant->closing_time) }}" required
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                            </div>
                        </div>
                                ...
                                @foreach($days as $value => $label)
                                    <label class="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                                        <input type="checkbox" name="working_days[]" value="{{ $value }}"
                                               {{ in_array($value, old('working_days', $restaurant->working_days ?? [])) ? 'checked' : '' }}
                                               class="w-4 h-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500">
                    ...
                    <div class="px-6 py-6 space-y-6">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                ...
                                @if($restaurant->image)
                                    <div class="mb-4">
                                        <img src="{{ asset('storage/' . $restaurant->image) }}" alt="Logo atual" class="object-cover w-32 h-32 rounded-lg">
                                    ...
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600">
                                            <label for="image" class="relative font-medium text-orange-600 bg-white rounded-md cursor-pointer hover:text-orange-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-orange-500">
                                                <span>{{ $restaurant->image ? 'Alterar arquivo' : 'Upload de arquivo' }}</span>
                            ...
                            <div>
                                ...
                                @if($restaurant->cover_image)
                                    <div class="mb-4">
                                        <img src="{{ asset('storage/' . $restaurant->cover_image) }}" alt="Capa atual" class="object-cover w-full h-32 rounded-lg">
                                        <p class="mt-2 text-sm text-gray-500">Imagem atual</p>
                                    </div>
                                @endif
                                <div class="flex justify-center px-6 pt-5 pb-6 mt-1 border-2 border-gray-300 border-dashed rounded-md">
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600">
                                            <label for="cover_image" class="relative font-medium text-orange-600 bg-white rounded-md cursor-pointer hover:text-orange-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-orange-500">
                                                <span>{{ $restaurant->cover_image ? 'Alterar arquivo' : 'Upload de arquivo' }}</span>
